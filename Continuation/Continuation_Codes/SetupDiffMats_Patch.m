...
N = 2001; % Number of mesh points, must be even! 
L = 40; % domain truncation
h = L/(N-1); 
r = (0:N-1)'*h;
...
M = sparse(1:N,1:N,sign(r-r0),N,N);
...
if n == 1
LM=sparse(N,N);
LM(1:N, 1:N) = D2 + R*D + e - (m)^2*(R.^2).*e; % d_rr + d_r/r + I-(i*m/r)^2
LM(1,1) = (2-((m)^2)/2)*D2(1,1) + 1; LM(1,2) = (2-((m)^2)/2)*D2(1,2); % 2d_rr at r= 0
LM(1:N,1:N) = -LM(1:N,1:N)^2; % -(1+laplacian)^2
else
...
mesh_params.N  = N; mesh_params.M  = M;

mesh_params.L  = L; mesh_params.m = m; mesh_params.e = e; mesh_params.k  = k;

mesh_params.r  = r; mesh_params.R = R; mesh_params.r0  = r0;
